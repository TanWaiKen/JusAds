...
export interface DistributionAccountsResponse {
  accounts: DistributionAccount[];
  message?: string;
}

export async function getDistributionAccounts(): Promise<DistributionAccountsResponse> {
  const res = await authenticatedFetch(`${API_BASE}/api/distribution/accounts`);
  const payload = await res.json().catch(() => ({})) as { accounts?: unknown; message?: unknown; error?: unknown };
  if (!res.ok) {
    throw new Error(
  ...
  const accounts = !Array.isArray(payload.accounts) ? [] : payload.accounts.flatMap((item): DistributionAccount[] => {
...
export interface AutofillRequest {
  user_prompt: string;
  current_design_type: string | null;
  current_values: Record<string, string>;
}

/** Populate guided-generation fields from an authenticated backend request. */
export async function autofillGenerationForm<T>(body: AutofillRequest): Promise<T> {
  const response = await authenticatedFetch(`${API_BASE}/api/generation/autofill`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    throw await toApiRequestError(response, "The setup assistant could not update the form.");
  }
